#!/usr/bin/env node
/** CI stub: ARC-PORT-052 densified off city centroid at 5dp (2026-09-14 ~18:45 MSK). */
import { readFileSync } from 'node:fs';

const EXPECTED = {
  'ARC-PORT-052': [67.7938459, -111.0127848],
};
...
for (const [id, [lat, lon]] of Object.entries(EXPECTED)) {
  ...
  const city = byId.get('ARC-CITY-051');
  if (city) {
    const [cx, cy] = city.c;
    if (y.toFixed(5) === cy.toFixed(5) && x.toFixed(5) === cx.toFixed(5)) {
      console.error('STILL_STACKED_5DP', id, 'with ARC-CITY-051');
      failed++;
    } else {
      console.log('UNSTACKED_5DP', id, 'vs ARC-CITY-051', cy, cx);
    }
  }
}
if (failed) process.exit(1);
console.log('check-port-city-centroid-densify-1845: pass');
