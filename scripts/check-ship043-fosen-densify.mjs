...
function parseCsv() {
  ...
  if (!line) throw new Error('ARC-SHIP-043 missing from CSV');
  const fields = [];
  let cur = '', inQ = false;
...
const feat = atlas.features.find((f) => (f.properties || {}).id === 'ARC-SHIP-043');
if (!feat) { console.error('FAIL: ARC-SHIP-043 missing from atlas'); process.exit(1); }
...
const ok =
  ...
  row.src.includes('Q1439358') &&
  (Math.abs(row.lat - softOld.lat) > 0.01 || Math.abs(row.lon - softOld.lon) > 0.01);
console.log(JSON.stringify({ id: 'ARC-SHIP-043', csv: row, atlas: [alon, alat], expectLat, expectLon, ok }, null, 2));
process.exit(ok ? 0 : 1);
